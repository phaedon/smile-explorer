...
inline void plotTarfVisualiser(ExplorerParams& params) {
  ...
  ImGui::SliderFloat("Notional (FOR)",
                     &notional,
                     0.0f,
                     1e7f,
                     "%.2f",
                     ImGuiSliderFlags_Logarithmic);
  ImGui::SliderFloat("Target (DOM)",
                     &target,
                     0.0f,
                     1e7f,
                     "%.2f",
  ...
  ImGui::SliderFloat(
      "Settlement Freq (years)", &settlement_frequency, 0.25f, 1.0f, "%.2f");
  settlement_frequency =
      std::max(0.25f, roundf(settlement_frequency / 0.25f) * 0.25f);
  ...
  displayCurrencyCombo(
      "Foreign Currency", foreign_idx, params, [&](Currency currency) {
        params.foreign_currency = currency;
      });
  displayCurrencyCombo(
      "Domestic Currency", domestic_idx, params, [&](Currency currency) {
        params.currency = currency;
      });

  if (ImGui::Button("Recalculate") || !initialized) {
    params.spot_price =
        (*params.global_currencies)(params.foreign_currency, params.currency)
            .value_or(1.0);
    const auto& foreign_curve =
        *params.global_rates->curves[params.foreign_currency];
    const auto& domestic_curve = *params.global_rates->curves[params.currency];

    avg_fwd = weightedAvgForward(params.spot_price,
                                 end_date_years,
                                 settlement_frequency,
                                 foreign_curve,
                                 domestic_curve);
    ...
    zero_npv_strike = findZeroNPVStrike(specs,
                                        params.spot_price,
                                        volatility,
                                        foreign_curve,
                                        domestic_curve,
    ...
    pricing_result = tarf.price(params.spot_price,
                                volatility,
                                settlement_frequency / 4,
                                num_paths,
                                foreign_curve,
                                domestic_curve);
    initialized = true;
  ...
  if (ImPlot::BeginPlot("NPV Distribution")) {
    if (!pricing_result.path_npvs.empty()) {
      auto [min_it, max_it] = std::minmax_element(
          pricing_result.path_npvs.begin(), pricing_result.path_npvs.end());
      double min_val = *min_it;
      double max_val = *max_it;
      double padding = (max_val - min_val) * 0.1;
      ImPlot::SetupAxisLimits(
          ImAxis_X1, min_val - padding, max_val + padding, ImPlotCond_Always);
    }
    ImPlot::PushStyleColor(ImPlotCol_Line, ImVec4(0, 0, 0, 1));
    ImPlot::PlotHistogram("NPV",
                          pricing_result.path_npvs.data(),
                          pricing_result.path_npvs.size(),
                          50);
